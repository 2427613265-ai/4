...
def main():
    ...
    c1 = [
        ...
        P(
            "t",
            "S4、再由跨中向两侧索塔对第i浇筑批次的所述非吊索区湿接缝进行逐批次、对称且跳仓浇筑；其中，i为正整数，i的起始值为1；所述跳仓浇筑为沿纵桥向间隔至少一个吊索节段浇筑，所述吊索节段为相邻两吊索之间区段；",
        ),
        P(
            "t",
    ...
    replacements = [
        ...
        (
            "第一阶段先浇筑预设跨中区段的非吊索区湿接缝，再由跨中向两侧索塔逐批次、对称且跳仓浇筑其余非吊索区湿接缝；待湿接缝混凝土达到预设强度后，第二阶段由跨中向两侧索塔对称、逐批次浇筑吊索区湿接缝，形成组合截面。",
            "第一阶段先浇筑预设跨中区段的非吊索区湿接缝，再按第i浇筑批次由跨中向两侧索塔对称跳仓浇筑，所述跳仓浇筑为沿纵桥向间隔至少一个吊索节段，若未浇完则将i+1赋值给i并返回；待湿接缝混凝土达到预设强度后，第二阶段按第N浇筑批次对称浇筑吊索区湿接缝，若未浇完则将N+1赋值给N并返回，形成组合截面。",
        ),
        (
            "铺设预制混凝土桥面板并形成湿接缝；将湿接缝划分为吊索区湿接缝与非吊索区湿接缝",
            "铺设预制混凝土桥面板形成湿接缝，划分为吊索区湿接缝与非吊索区湿接缝",
        ),
    ]
