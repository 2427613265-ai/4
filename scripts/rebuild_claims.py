...
def main():
    ...
    c1 = [
        ...
        P(
            "t",
            "S2、将所述湿接缝划分为吊索区湿接缝和非吊索区湿接缝；所述吊索区湿接缝为位于吊索的中心线两侧预设范围内的湿接缝；所述非吊索区湿接缝为除所述吊索区湿接缝以外的湿接缝；",
        ),
        P(
            "t",
            "S3、开始第一阶段浇筑，先浇筑预设跨中区段的所述非吊索区湿接缝；其中，所述跨中区段为以跨中为中心对称延伸的区段；",
        ),
        P(
            "t",
            "S4、再由跨中向两侧索塔对第i浇筑批次的所述非吊索区湿接缝进行逐批次、对称且跳仓浇筑；其中，i≥1，i为正整数，i的起始值为1；所述跳仓浇筑为沿纵桥向间隔至少一个吊索节段浇筑，所述吊索节段为相邻两吊索之间区段；",
        ),
        P(
            "t",
        ...
        P(
            "t",
            "S6、判断所述第一阶段的湿接缝混凝土是否达到预设强度，若否，待所述湿接缝混凝土达到预设强度后进入步骤S7；若是，进入步骤S7；",
        ),
        P(
            "t",
            "S7、开始第二阶段浇筑，由跨中向两侧索塔对称浇筑第N浇筑批次的所述吊索区湿接缝；其中，N≥1，N为正整数，N的起始值为1；",
        ),
        P(
            "t",
            "S8、判断第N浇筑批次完成后是否仍存在尚未浇筑的吊索区湿接缝，若是，将N+1赋值给N，返回步骤S7；若否，全桥湿接缝浇筑完成，形成组合截面。",
        ),
    ]
    claims.extend(c1)
    claims.append(E)
    ...
    c2 = [
        P(
            "t",
            "2.根据权利要求1所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，所述S3中先浇筑预设跨中区段的所述非吊索区湿接缝包括以下步骤：",
        ),
        P(
            "t",
        ...
        P(
            "t",
            "S32、向两侧索塔方向对称选取第q次尚未浇筑的非吊索区湿接缝并浇筑，获得浇筑后的当前主缆水平张力",
            "m",
            H1,
            "t",
            "；",
        ...
        P(
            ...
            H0,
            "t",
            "是否小于或等于15%，若是，将q+1赋值给q，返回步骤S32；若否，则判定跨中区段浇筑完成，并进入步骤S4。",
        ),
    ]
    claims.extend(c2)
    claims.append(E)
    ...
    c3 = [
        P(
            "t",
            "3.根据权利要求2所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，所述S4中由跨中向两侧索塔对第i浇筑批次的所述非吊索区湿接缝进行逐批次、对称且跳仓浇筑包括以下步骤：",
        ),
        P(
            "t",
    ...
    c4 = [
        P(
            "t",
            "4.根据权利要求1所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，所述S6中，在所述第一阶段的湿接缝混凝土达到预设强度、且主缆线形达到预设指标后进入步骤S7；所述预设指标为：所述第一阶段连续三个浇筑批次中，每一批次结束后的跨中竖向位移变化量Δv均满足|Δv|≤3mm，且每一批次结束后的索塔塔顶纵向位移变化量均≤5mm。",
        ),
    ]
    claims.extend(c4)
    claims.append(E)
    ...
    c5 = [
        P(
            "t",
            "5.根据权利要求1所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，所述S7中，在第N浇筑批次为所述第二阶段最末一批次之前，先执行以下步骤：",
        ),
        P(
            "t",
    ...
    c6 = [
        P(
            "t",
            "6.根据权利要求1所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，还包括以下步骤：",
        ),
        P(
            "t",
    ...
    c7 = [
        P(
            "t",
            "7.根据权利要求6所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，所述预设阈值为L/10000，L为所述主缆跨度；在所述",
            "m",
            Wenv,
            "t",
    ...
    c8 = [
        P(
            "t",
            "8.根据权利要求1至7中任一项所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，所述湿接缝混凝土的强度等级不低于所述预制混凝土桥面板的强度等级。",
        ),
    ]
    claims.extend(c8)
    claims.append(E)
    ...
    c10 = [
        P(
            "t",
            "10.根据权利要求1所述的悬索桥钢混组合梁原位叠合浇筑施工方法，其特征在于，所述第一阶段分为10-12个浇筑批次完成，每一批次的浇筑方量与设计方量之差的绝对值不超过设计方量的5%。",
        ),
    ]
    claims.extend(c10)
    ...
    invention = [
        ...
        P(
            "t",
            "S2、将所述湿接缝划分为吊索区湿接缝和其余非吊索区湿接缝；所述吊索区湿接缝为位于吊索中心线两侧预设范围内的湿接缝；所述非吊索区湿接缝为除所述吊索区湿接缝以外的湿接缝；",
        ),
        P(
            "t",
            "S3、开始第一阶段浇筑，先浇筑预设跨中区段的所述非吊索区湿接缝；其中，所述跨中区段为以跨中为中心对称延伸的区段；",
        ...
        P(
            "t",
            "S4、再由跨中向两侧索塔对第i浇筑批次的所述非吊索区湿接缝进行逐批次、对称且跳仓浇筑；其中，i≥1，i为正整数，i的起始值为1；所述跳仓浇筑为沿桥纵向间隔至少一个吊索节段浇筑，所述吊索节段为相邻两吊索之间区段；",
        ),
        P(
            "t",
            "S5、判断第i浇筑批次完成后是否仍存在尚未浇筑的非吊索区湿接缝，若是，将i+1赋值给i，返回步骤S4；若否，进入步骤S6；",
        ...
        P(
            "t",
            "S8、判断第N浇筑批次完成后是否仍存在尚未浇筑的吊索区湿接缝，若是，将N+1赋值给N，返回步骤S7；若否，全桥湿接缝浇筑完成并形成组合截面。",
        ),
        P(
            "t",
        ...
        P(
            "t",
            "进一步地，所述S6中，所述第二阶段在主缆线形达到预设指标后进入步骤S7；所述预设指标为：所述第一阶段连续三个浇筑批次中，每一批次结束后的跨中竖向位移变化量Δv均满足|Δv|≤3mm，且每一批次结束后的索塔塔顶纵向位移变化量均≤5mm。",
        ),
        P(
            "t",
            "进一步地，所述S7中，在第N浇筑批次为所述第二阶段最末一批次之前，复测主缆跨中、1/4跨和3/4跨的标高，得到各测点的实测标高，并计算各测点实测标高与该测点当前施工阶段目标标高之差Δ",
            "m",
            hk,
            "t",
    ...
    replacements = [
        ...
        (
            ...
            "在步骤S31于浇筑前一次获得H₀，作为本轮跨中扩展的张力基准；此后按第q次选取并浇筑，浇筑完成后获得H₁，并计算(H₁−H₀)/H₀。若(H₁−H₀)/H₀≤15%，将q+1赋值给q并返回步骤S32，H₀保持不变；若(H₁−H₀)/H₀>15%，判定跨中区段浇筑完成，进入步骤S4。",
        ),
    ]
